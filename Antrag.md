...
## Untertitel
Entwicklung eines auf Natursprache (NLP) basierenden, IFC-konformen KI-Frameworks zur Analyse und Orchestrierung bestehender sowie Generierung neuer Automatisierungsskripte für BIM-Umgebung.

## Projektbeschreibung
Die deutsche Bauwirtschaft ist geprägt von zahlreichen kleinen und mittleren Planungsbüros (KMU), die einen erheblichen Rückstand bei der Digitalisierung ihrer Arbeitsprozesse aufweisen. Insbesondere fehlen vielen KMU Ressourcen für Forschung und Entwicklung, um moderne Automatisierungstechnologien einzuführen und zu nutzen. Gängige KI-basierte Werkzeuge liegen zumeist als proprietäre Plug-ins einzelner Softwareproduzenten vor. Dies führt zu einem Vendor-Lock-in und erschwert die Interoperabilität mit fachspezifischen Planungstools, die auf dem offenen IFC-Standard basieren. Gleichzeitig bleibt vorhandenes internes Wissen in Form automatisierter Skripte (z.B. für Dynamo oder Python) isoliert und ungenutzt. Das mangelnde Vertrauen in intransparente "Black-Box"-KI-Systeme stellt eine weitere Hürde dar.
Dieses Vorhaben zielt darauf ab, KI-basierte Verfahren zu erforschen, mit denen Planungsanforderungen und -aufgaben in automatisierte BIM-Workflows überführt werden können. Als Demonstrator wird ein software-unabhängiger Digital Mate prototypisch erstellt. Dieser Digital Mate soll Planungsaufgaben in natürlicher Sprache entgegennehmen und mithilfe einer intelligenten Orchestrierung vorhandene oder neu erstellte Skripte in ausführbare Workflows übersetzen. Die resultierenden Workflows werden prozesssicher auf beliebige IFC-basierte BIM-Modelle angewendet. Dabei werden Explainable-AI-Techniken integriert und nutzerseitige Kontrollmechanismen entwickelt. Mit dieser Methodik wird die systematische Integration von KI in Planungsprozesse untersucht und exemplarisch validiert.

## Einordnung in den Stand des Wissens und der Praxis und Beitrag Ihres Projekts
Der Stand der Technik bei KI im Bauwesen fokussiert auf generative Einzelaufgaben in proprietären Ökosystemen. Großen Sprachmodellen (LLMs) fehlt der BIM-Modellkontext, um komplexe, mehrstufige Planungsprozesse zu orchestrieren. Bestehende skriptbasierte Automatisierungslösungen erfordern hohes Expertenwissen, während KI-Werkzeuge fehlen, die nativ auf dem offenen IFC-Standard operieren und zugleich vorhandene Skript-Bibliotheken eines Unternehmens integrieren. Zudem mangelt es an Methoden der "Explainable AI" (XAI), die KI-Handlungen für Planer transparent und verifizierbar machen.
Das Projekt liefert drei zentrale Beiträge zur Überwindung dieser Defizite:

1.  **IFC-native Methodik gegen Vendor-Lock-in:** Durch die native Operation auf dem offenen IFC-Standard wird eine herstellerunabhängige Schnittstelle geschaffen, die in den heterogenen Softwarelandschaften von Planungsbüros nahtlos einsetzbar ist.
2.  **Dynamische Workflow-Synthese aus isolierten Skripten:** Ein neuartiges System analysiert bestehende Skripte (Dynamo, Python, C#), extrahiert Metadaten und legt sie in einer internen Bibliothek ab. Daraus werden bei Nutzeranfragen passgenaue, mehrstufige Workflows automatisch generiert – eine kohärente Verknüpfung von Bestands- und Neuskripten.
3.  **Vertrauen durch Trustworthy & Explainable AI:** Wir entwickeln Methoden für eine prozesssichere Automatisierung. Mittels visueller "Diff"-Vergleiche, Regelprüfungen und "Trockenlauf"-Simulationen wird die Modellintegrität sichergestellt und das Vertrauen der Nutzer in die KI-Anwendung gefördert.

## Ziel(e) des Projekts und erwartete Effekte
**Spezifische Forschungsziele:**
* **Entwicklung einer Methodik zur KI-Orchestrierung:** Kernziel ist die Erarbeitung einer Methodik, die hoch-abstrakte Planungsanweisungen in ausführbare Workflows übersetzt und so die Lücke zwischen menschlicher Intention und bestehenden Automatisierungsskripten schließt.
...
* **Erarbeitung und Test von "Explainable AI"-Paradigmen für BIM:** Es werden spezifische Methoden der erklärbaren KI (XAI) für den BIM-Kontext evaluiert, um durch geeignete Interaktionen Vertrauen und prozesssichere Kontrolle zu gewährleisten.
...
* **Erhöhung der Transparenz und des Vertrauens in KI:** Die erforschten XAI-Methoden holen KI aus der "Black-Box" und schaffen die Grundlage für vertrauenswürdige Assistenzsysteme, was die Akzeptanz in der Praxis erhöht.
...
Die primäre Barriere für die breite Anwendung von KI-gestützter Automatisierung in der heterogenen deutschen Planungslandschaft ist nicht der Mangel an automatisierbaren Aufgaben, sondern die Dominanz geschlossener, softwarespezifischer Systeme und ein Vertrauensdefizit aufgrund deren "Black-Box"-Charakters. Wir postulieren, dass ein softwareunabhängiger, transparenter KI-Co-Pilot (Digital Mate), der auf Basis natürlicher Sprache nutzereigene Skripte orchestriert, die Effizienz und Akzeptanz signifikant steigern kann, indem er den Anwender in die volle Kontrolle über einen verifizierbaren Prozess versetzt.
...
2.  Mit welchen Methoden der statischen und dynamischen Code-Analyse lässt sich das in imperativen Skripten (Python, Dynamo) implizit enthaltene prozedurale Wissen extrahieren und in eine explizite, semantische Repräsentation (z.B. einen Wissensgraphen) überführen, die eine Rekombination zur Lösung komplexer, übergeordneter Probleme ermöglicht?
3.  Welche Interaktions- und Visualisierungsparadigmen (XAI) sind notwendig, um einen verifizierbaren und vertrauenswürdigen Human-in-the-Loop-Prozess zu etablieren, der die Effizienz der Automatisierung mit der Notwendigkeit menschlicher Kontrolle und Validierung in Einklang bringt? Wie beeinflussen diese Paradigmen die Akzeptanz und Fehleranfälligkeit des Gesamtsystems?
...
3.  **Bildungs- und Forschungseinrichtungen:** Die Projektergebnisse, insbesondere die Methodik zur intelligenten Orchestrierung von Skripten und das XAI-Framework, liefern wertvolle Erkenntnisse und Lehrmaterial für die Ausbildung und weitere Forschung im Bereich der angewandten KI im Bauwesen.
